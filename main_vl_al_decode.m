...
% 读取随机工期数据
% fp_duration=['D:\研究生资料\SRLP-PS汇总\数据和代码_final\随机工期\',num2str('B2'),'\J',actNumber,'\',actno,'.txt'];
...
% 写入文件路径
fpathRoot=['C:\Users\ASUS\Desktop\测试实验20211014\DE\J',actNumber,'\'];
...
% 迭代中的种群
new_implementList=zeros(popsize,actNo+2);
new_alList = zeros(popsize,actNo);
new_decodeList = zeros(popsize,actNo);
% 初始参数设置
A_min = 0.3;
A_max = 1.5;
r_min = 0.1;
...
while nr_schedules<end_schedules
    ...
    for pop = 1:popsize 
        ...
        temp_value = [implementList(r_list(1),actNo+1) implementList(r_list(2),actNo+1) implementList(r_list(3),actNo+1)];
        % 按目标函数值排序，降序
        [~,b]=sort(temp_value);
        r_list = r_list(b);
        r1 = r_list(1);
        ...
        r3 = r_list(3);
        % 将活动列表转化为优先值
        al_prio_r1 = transmute_prio(alList(r1,:),actNo);
        al_prio_r2 = transmute_prio(alList(r2,:),actNo);
        al_prio_r3 = transmute_prio(alList(r3,:),actNo);
        % 随机数向量
        rand_implement = rand(1,actNo);
        rand_al = rand(1,actNo);
        %% 变异
        % 新的变异个体
        ...
        flag = 0;
        implement_r1 = implementList(r1,1:actNo)+rand(1,actNo);
        implement_r2 = implementList(r2,1:actNo)+rand(1,actNo);
        implement_r3 = implementList(r3,1:actNo)+rand(1,actNo);
        code_r1 = decodeList(r1,1:actNo)+rand(1,actNo);
        code_r2 = decodeList(r2,1:actNo)+rand(1,actNo);
        code_r3 = decodeList(r3,1:actNo)+rand(1,actNo);
        
        current_code = decodeList(pop,1:actNo)+rand(1,actNo);
        current_implement = implementList(pop,1:actNo)+rand(1,actNo);
         %当前活动列表转化为优先值
        current_prio = transmute_prio(alList(pop,:),actNo);
        for i = 1:actNo 
            if rand>r_m
                flag = 1;
       ...
       %% 将试验向量转化执行列表和活动列表\
        ...
        if expected_obj<implementList(pop,actNo+1)
            new_implementList(pop,:) = new_implement;
            new_alList(pop,:) = new_al;     
            new_decodeList(pop,:)=new_code;
        else
            new_implementList(pop,:) = implementList(pop,:);
            new_alList(pop,:) = alList(pop,:);
            new_decodeList(pop,:) = decodeList(pop,:);
        end
...
%         nr_schedules=nr_schedules+rep; 
    end % 生成新个体结束 一次迭代结束
    nr_schedules=nr_schedules+popsize;
    %% 选择B个最好的个体
    p=new_implementList;
    %  选择最好的pop个体作为父代，按照目标函数值升序排（最小的在前面）
    [a,fitIndex]=sort(new_implementList(:,actNo+1));
    fitIndex=fitIndex(1:elit_p);
    eliteSet_implement=p(fitIndex,:);
    eliteSet_AL=new_alList(fitIndex,:);
    eliteSet_code = new_decodeList(fitIndex,:);
    %% 局部搜索
    for i=1:elit_p
        local_count = local_count+1;
        temp_code = eliteSet_code(i,:);
        temp_VL=eliteSet_implement(i,:);
        temp_AL=eliteSet_AL(i,:);
        new_implement=temp_VL;       
        [projRelation_i,nrpr_i,nrsu_i,su_i,pred_i]=updateRelation(projRelation,nrpr,nrsu,su,pred,choiceList,new_implement(1:actNo),actNo);
        % 改code
        temp_code = decode_mutation(temp_code,actNo,p_mutation);
        % 与紧邻的活动移动
...
%          nr_schedules=nr_schedules+rep; 
    ...
    implementList = new_implementList;
    alList = new_alList;
    decodeList = new_decodeList;
end % 迭代结束
...
% outFile=[fpathRoot,num2str(end_schedules),'_sch_de_',setName,'_dt_',dt,'_main_vl_decode_value_ssgs_8_',num2str(rep),'.txt'];
% dlmwrite(outFile,outResults,'-append', 'newline', 'pc',  'delimiter', '\t');
% % 写入gap
% outResults1 = [act, end_gap];
% outFile1=[fpathRoot,num2str(end_schedules),'_sch_de_',setName,'_dt_',dt,'_main_vl_decode_gap_ssgs_8_',num2str(rep),'.txt'];
