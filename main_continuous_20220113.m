...
%% 初始种群转化为连续个体
prio_al_list = zeros(popsize,actNo);
prio_vl_list = zeros(popsize,actNo+2);
prio_dl_list = zeros(popsize,actNo);
for pop=1:popsize
    prio_al_list(pop,:) = transmute_prio(alList(pop,:),actNo);
    prio_vl_list(pop,1:actNo) = implementList(pop,1:actNo)*rand(1,actNo);
    prio_vl_list(pop,actNo+1:actNo+2) = implementList(pop,actNo+1:actNo+2);
    prio_dl_list(pop,:) = decodeList(pop,:)*rand(1,actNo);
end
%% 迭代
% 时间终止条件
end_time = 30;
...
%                 best_decode_pro = best_decode(1:actNo)+rand(1,actNo);
                ...
                randi_index = randi([1,popsize*0.1],1);
                best_part_temp = best_part(randi_index);
                ...
                mutation_implement(i) = current_implement(i)+A*(best_implement_pro(i)-current_implement(i))+(A/2)*(implement_r1(i)-implement_r2(i));
                mutation_decode(i) = current_code(i)+A*(best_decode_pro(i)-current_code(i))+(A/2)*(code_r1(i)-code_r2(i));
                mutation_al(i) = current_prio(i)+A*(best_prio(i)-current_prio(i))+(A/2)*(al_prio_r1(i)- al_prio_r2(i));
