...
% 超过截止日期也计算资源波动
% 惩罚，最大的偏差*gap_d
function [expected_obj,time_pro,gap_d]=evaluate_abs_consider_gap_code(AL,rep,implement,req,resNumber,nrpr,pred,nrsu,su,deadline,resNo,actNo,stochatic_d,code)
% deadline与实际的差值
gap_d =[];
% 满足截止日期的次数
p=0;
...
%     [schedule, u_kt] = SSGS_PSGS1(AL,implement,req,resNumber,d,nrpr,pred,deadline,resNo,actNo,code);
    ...
    if scheduleFeasible(schedule,actNo,nrsu,su,implement,d)==1 && resourceFeasible(u_kt1,resNumber)==1 
        if schedule(actNo)<=deadline
            ...
            flag = 1;
            temp_gap_d = schedule(actNo)-deadline;
            gap_d = [gap_d temp_gap_d];
        end
        % 情景下的目标函数
        scen_obj = 0;
