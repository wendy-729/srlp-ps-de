...
% 写入文件路径
fpathRoot=['C:\Users\ASUS\Desktop\测试实验20211028\DE\J',actNumber,'\'];
...
best_implement_pro(actNo+1)=best_implement(actNo+1);
best_implement_pro(actNo+2)=best_implement(actNo+2);
best_decode_pro = best_decode(1:actNo)+rand(1,actNo);
%% 初始化
% 将活动列表转化为优先值列表
prioList = zeros(popsize,actNo);
for i=1:popsize
    prioList(i,:) = transmute_prio(alList(i,:),actNo);
end
% 执行列表转化为实数
implementPrio = zeros(popsize,actNo+2);
for i=1:popsize
    implementPrio(i,1:actNo)=implementList(i,1:actNo)+rand(1,actNo);
    implementPrio(i,actNo+1)=implementList(i,actNo+1);
    implementPrio(i,actNo+2)=implementList(i,actNo+2);
end
% 编码列表转化为实数
decodePrio= zeros(popsize,actNo);
for i=1:popsize
    decodePrio(i,:)=decodeList(i,:)+rand(1,actNo);
end
new_implementPrio = implementPrio;
new_prio = prioList;
new_decodePrio = decodePrio;
%% 迭代
end_schedules=2000;
...
while nr_schedules<end_schedules
    ...
    for pop = 1:popsize 
        ...
        temp_value = [new_implementPrio(r_list(1),actNo+1) new_implementPrio(r_list(2),actNo+1) new_implementPrio(r_list(3),actNo+1)];
        % 按目标函数值排序，降序
        [~,b]=sort(temp_value);
        r_list = r_list(b);
        r1 = r_list(1);
        ...
        al_prio_r1 = new_prio(r1,:);
        al_prio_r2 = new_prio(r2,:);
        al_prio_r3 = new_prio(r3,:);
        implement_r1 = new_implementPrio(r1,1:actNo);
        implement_r2 = new_implementPrio(r2,1:actNo);
        implement_r3 = new_implementPrio(r3,1:actNo);
        code_r1 = new_decodePrio(r1,:);
        code_r2 = new_decodePrio(r2,:);
        code_r3 = new_decodePrio(r3,:);
       %% 变异
        % 新的变异个体
        ...
        current_code = new_decodePrio(pop,1:actNo);
        current_implement = new_implementPrio(pop,1:actNo);
        current_prio = new_prio(pop,:);
        for i = 1:actNo 
            if rand>r_m
                flag = 1;
                ...
                mutation_implement(i) = current_implement(i)+A*(best_implement_pro(i)-current_implement(i))+A*(implement_r1(i)-implement_r3(i));
                mutation_decode(i) = current_code(i)+A*(best_decode_pro(i)-current_code(i))+A*(code_r1(i)-code_r3(i));
                mutation_al(i) = current_prio(i)+A*(best_prio(i)-current_prio(i))+A*(al_prio_r1(i)- al_prio_r3(i));
            end
        end
       %% 交叉
        % 目标个体,不是前面变异用的三个个体
...
%             target_al = al_prio_r1;
       ...
       %% 将试验向量转化执行列表和活动列表
        ...
        trial_implement(actNo+1)=expected_obj;
        trial_implement(actNo+2)=time_pro; 

        if expected_obj<new_implementPrio(pop,actNo+1)
            new_implementPrio(pop,:) = trial_implement;
            new_prio(pop,:) = trial_al;     
            new_decodePrio(pop,:)=trial_decode;
        end
        if  expected_obj<best_implement(actNo+1)
            best_decode_pro = trial_decode;
            best_implement_pro = trial_implement;
            best_prio = trial_al;
...
outFile=[fpathRoot,num2str(end_schedules),'_sch_de_',setName,'_dt_',dt,'_main_vl_decode_value_','_all_prio_',num2str(rep),'.txt'];
