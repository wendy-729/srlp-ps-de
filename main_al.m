...
while nr_schedules<end_schedules
    ...
    for pop = 1:popsize 
        ...
        rand_code = rand(1,actNo);
        
        al_prio_r1 = new_alList(r1,:);
        al_prio_r2 = new_alList(r2,:);
        al_prio_r3 = new_alList(r3,:);
        implement_r1 = new_implementList(r1,1:actNo);
        implement_r2 = new_implementList(r2,1:actNo);
        implement_r3 = new_implementList(r3,1:actNo);
        code_r1 = new_decodeList(r1,1:actNo);
        code_r2 = new_decodeList(r2,1:actNo);
        code_r3 = new_decodeList(r3,1:actNo);
...
%         code_r1 = new_decodeList(r1,1:actNo)+rand(1,actNo);
%         code_r2 = new_decodeList(r2,1:actNo)+rand(1,actNo);
%         code_r3 = new_decodeList(r3,1:actNo)+rand(1,actNo);
       %% 变异
        % 新的变异个体
...
%         current_prio = transmute_prio(new_alList(pop,:),actNo);
        current_code = new_decodeList(pop,1:actNo);
        current_implement = new_implementList(pop,1:actNo);
        current_prio =new_alList(pop,:);
        for i = 1:actNo 
            if rand>r_m
                flag = 1;
                mutation_implement(i) = implement_r1(i)+A*rand_implement(i)*(implement_r2(i)-implement_r3(i));
                mutation_decode(i) = code_r1(i)+A*rand_code(i)*(code_r2(i)-code_r3(i));
                mutation_al(i) = al_prio_r1(i)+A*rand_al(i)*(al_prio_r2(i)- al_prio_r3(i));
...
%                 best_decode_pro = best_decode(1:actNo)+rand(1,actNo);
                best_prio = best_al;
                best_implement_pro = best_implement(1:actNo);
                best_decode_pro = best_decode(1:actNo);
                
                mutation_implement(i) = current_implement(i)+A*rand_implement(i)*(best_implement_pro(i)-current_implement(i));
                mutation_decode(i) = current_code(i)+A*rand_code(i)*(best_decode_pro(i)-current_code(i));
                mutation_al(i) = current_prio(i)+A*rand_al(i)*(best_prio(i)-current_prio(i));
            end
        end
       %% 交叉
        % 目标个体,不是前面变异用的三个个体
...
% outFile=[fpathRoot,num2str(end_schedules),'_sch_de_',setName,'_dt_',dt,'_main_vl_decode_value_no_local','_',num2str(rep),'.txt'];
